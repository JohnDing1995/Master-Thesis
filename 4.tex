...
In this section, we describe our consideration when select tools used in the continuously deliver pipeline.
...
\begin{figure}[h]
    ...
    \label{fig:overview}
\end{figure}
Figure \ref{fig:overview} shows the 5 stages in our pipeline that shown in the Jenkins dashboard. The bottom part of this Figure shows the task distribution between the master node and agent nodes. 
...
\par
The considerations behind to our design are that the first 2 steps take most of the time in our pipeline and according to Figure \ref{fig:pipeline} runs more frequently than other steps \footnote{The reason will be discussed in next section "Pipeline Workflow"}. The running time will be further extended when building a larger project. These 2 stages will be the bottleneck of the pipeline if we have it on the master mode. So we need to offload these steps to Jenkins agents for better performance.
\par
...
\par
\label{deploy}
The last step of the pipeline is deployment, the pipeline pull image in ECR that we pushed in the last stage, and then deploy it to the deployment environment. 
\par
...
\subsection{Continuous Delivery Pipeline with AWS }
The workflow of our continuous delivery pipeline is the same with the pipeline in \ref{CD}. Instead of Jenkins which is server-based, we build the pipeline with AWS CodePipeline. The steps in the CodePipeline shows in 
